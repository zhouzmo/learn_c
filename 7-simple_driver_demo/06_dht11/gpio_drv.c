...
static void key_timer_expire(unsigned long data)
{
	...
	printk(" %s %d, 中断产生次数 %d\n", __FUNCTION__, __LINE__, g_dht11_irq_cnt);
	parse_dht11_datas();
}
...
static ssize_t dht11_read(struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	// 10ms 后无论是否发生足够的中断次数，也使用定时器触发事件唤醒
...
static void parse_dht11_datas(void)
{
	...
	printk(" %s line:%d, datas[0]=%d datas[1]=%d datas[2]=%d， datas[3]=%d，datas[4]=%d，crc=%d\n", __FUNCTION__, __LINE__,
		   datas[0], datas[1], datas[2], datas[3], datas[4], crc);
	printk(" %s line:%d, 中断产生次数 %d\n", __FUNCTION__, __LINE__, g_dht11_irq_cnt);
