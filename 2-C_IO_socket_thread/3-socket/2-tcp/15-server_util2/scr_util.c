 ...
 */
int sckServer_init(void **handle, int conntime, int sendtime, int recvtime, int nConnNum)
{
    int ret;
    ...
    tmp->sendtime = sendtime;
    tmp->recvtime = recvtime;
    tmp->listenfd = listenfd;
    *handle = tmp;
    LOGGER("初始化 handle success，服务端创建 success\n");
 ...
 */
int sckServer_AcceptConn(void *handle, char *ip, int *port, int *conn)
{
    int ret = 0;
    if (handle == NULL)
    {
        ret = Sck_ErrParam;
        printf("err: handle == NULL  %d\n", Sck_ErrParam);
        return ret;
    }
    ...
    myConn = accept_timeout(tmp->listenfd, &peeraddr, tmp->conntime);
 ...
 */
int sck_send(void *handle, int connfd, unsigned char *data, int datalen)
{
    int ret = 0;
 ...
 */
int sck_recv(void *handle, int connfd, unsigned char *out, int *outlen)
{
    LOGGER("start\n");
    int ret = 0;
